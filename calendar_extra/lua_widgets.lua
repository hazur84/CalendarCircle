...
-- Number of weeks per year --- 
  create_circle(cr,w-x_rel_pos,h-y_rel_pos, 52.0, 1, 3.5, 325, 13, CAIRO_OPERATOR_CLEAR, -4.5, tonumber(conky_parse('${exec date +%V}')), '')
  
-- Number of days in a month ---
  create_circle(cr,w-x_rel_pos,h-y_rel_pos, conky_parse('${exec cal |egrep -v [a-z] |wc -w}'), 1, 3.5, 300, 13,CAIRO_OPERATOR_CLEAR, -4.5,tonumber(conky_parse('${exec date +%d}')), '')

-- Numer of Months per year ---  
  local month = {"January", "February", "Mach", "April", "May", "June", "July", "August", "September", "October", "November", "December"}
  create_circle(cr,w-x_rel_pos,h-y_rel_pos, 12, 1, 3.5, 350, 13, CAIRO_OPERATOR_CLEAR, -4, tonumber(conky_parse('${exec date +%m}')), month, 5.5)

-- Number of days of week --
-- function create_circle(cr,w,h, elements, distance_between_blocks, two_number_degree, radius, line_width, operator, radius_shift_for_text, current, days, shift_days_distance)
 
  local days = {"Monday", "Tuesday", "Wednesday","Thursday", "Friday", "Saturday", "Sunday"}
  create_circle(cr,w-x_rel_pos,h-y_rel_pos, 7, 1, 3.5, 275, 13, CAIRO_OPERATOR_CLEAR, -4, tonumber(conky_parse('${exec date +%u}')), days, 8.5)

-- Number of hors per day ---
  create_circle(cr,w-x_rel_pos,h-y_rel_pos, 23, 1, 7, 250, 13, CAIRO_OPERATOR_CLEAR, -4.5, tonumber(conky_parse('${exec date +%H}')), '')

-- Number of Hours per sun ---
  create_circle(cr,w-x_rel_pos,h-y_rel_pos, 11, 1, 4, 225, 13, CAIRO_OPERATOR_CLEAR, -4.5, tonumber(conky_parse('${exec date +%I}')), '')

-- Number of minutes per hour ---
  create_circle(cr,w-x_rel_pos,h-y_rel_pos, 59, 1, 4, 200, 13, CAIRO_OPERATOR_CLEAR, -4.5, tonumber(conky_parse('${exec date +%M}')), '')
...
--- Clock ---
  ...
  --cairo_move_to(cr, (w-x_rel_pos)/2-100,(h-y_rel_pos)/2+20)
